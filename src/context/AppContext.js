...
export function AppProvider({ children }) {
  ...
  const logout = () => { setUser(null); setFavorites([]); setReservations([]); };
  ...
  const addReservation = (reservation) => {
    const newRes = { ...reservation, id: Date.now().toString(), status: 'pending', createdAt: new Date().toISOString() };
  ...
  const sendMessage = (venueId, text, sender) => {
    const msg = { id: Date.now().toString(), text, sender, time: new Date().toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' }) };
    setMessages(prev => ({ ...prev, [venueId]: [...(prev[venueId] || []), msg] }));
  };

  return (
    <AppContext.Provider value={{ user, login, register, logout, favorites, toggleFavorite, reservations, addReservation, updateReservationStatus, messages, sendMessage }}>
      {children}
    </AppContext.Provider>
  );
}
